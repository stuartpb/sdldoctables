...
comments={
  ...
  --Above this line is from the comments in SDL_scancode.h, replicated on the wiki page.
  --below this line is stuff modified from the SDLKey wiki page
...
--convert all the keycodes that are statically defined
--single-character strings to their equivalent bytes
...
--table of symbols in order
local symbols={}
--populate table with keys
...
print"||Symbol||Scancode||Keycode||Name||"
for i=1,#symbols do
  local symbol=symbols[i]
  local scancode=scancodes["SDL_SCANCODE_"..symbol]

  if     scancode == 4 then
    print[[||||<bgcolor="#EDEDED">''These values are mapped from usage page 0x07 (USB keyboard page)''||]]
  elseif scancode == 130 then
    print[[||||<bgcolor="#EDEDED">''The keys below are commented out in SDL_scancode.h''||]]
  elseif scancode == 134 then
    print[[||||<bgcolor="#EDEDED">''The keys above are commented out in SDL_scancode.h''||]]
  elseif scancode == 258 then
    print[[||||<bgcolor="#EDEDED">''These values are mapped from usage page 0x0C (USB consumer page)''||]]
  elseif scancode == 275 then
    print[[||||<bgcolor="#EDEDED">''These are values that Christian Walther added (for mac keyboard?)''||]]
  end

  local keycode=keycodes["SDLK_"..(#symbol==1 and string.lower(symbol) or symbol)]
  local kcdesc
  if keycode then
    --return different results for converted scancodes
    if keycode > 2^31 then
      ...
      --Lua also doesn't handle \b or \t correctly
      elseif kcchar=='\b' then
        kcchar="'\\b'"
      elseif kcchar=='\t' then
        kcchar="'\\t'"
      else
        kcchar=string.gsub(string.format("%q",kcchar),'"',"'")
      end
